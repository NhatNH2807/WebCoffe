...
public class AuthController : ControllerBase
{
    ...
    public AuthController(WebCoffeeNhatContext context, JwtService jwtService, ILogger<AuthController> logger,
    ...
    public IActionResult login([FromBody] LoginRequest request)
    {
        User? user = _context.Users.FirstOrDefault(
            u => u.Username == request.Username && u.IsLocked == false);
        if (user != null)
        {
            if (BCrypt.Net.BCrypt.Verify(request.Password, user.Password))
            {
                ...
                {
                    _logger.LogInformation($"LOGIN USERNAME: {request.Username} >>> SUCCESS");
                    return Ok(new BaseResponse<string>
                    {
                        code = "200",
                        message = "Thanh cong",
                        data = _jwtService.GenerateJSONWebToken(new UserLoginPrinciple
                        {
                ...
                SendActiveAccount(user.FullName, user.Username, user.Id);
                _logger.LogInformation($"LOGIN USERNAME: {request.Username} >>> FAIL NOT ACTIVE");
                return Ok(new BaseResponse<string>
                {
                    code = HttpStatusCode.BadRequest.GetHashCode().ToString(),
                    message = "Tai khoan chua duoc kich hoat vui long vao mail de kich hoat tai khoan",
                });
            }
        }
    ...
    public IActionResult activeAccount(string tokenValue)
    {
        ...
        Token? token = _context.Tokens.FirstOrDefault(t => t.TokenValue == tokenValue);
        if (token == null)
        {
            return BadRequest(new BaseResponse<object>
            {
                code = HttpStatusCode.BadRequest.GetHashCode().ToString(),
                message =
                    "Token không chính xác vui lòng vào mail của bạn để kích hoạt hoặc đăng nhập để nhận lại mail xác nhận mới"
            });
        }
        ...
        if (token.IsUsed == true)
        {
            return BadRequest(new BaseResponse<object>
            {
                code = HttpStatusCode.BadRequest.GetHashCode().ToString(),
                message =
                    "Token đã được sử dụng vui lòng vào mail của bạn để kích hoạt hoặc đăng nhập để nhận lại mail xác nhận mới"
            });
        }
        ...
        if (token.ExpiredTime < DateTime.UtcNow)
        {
            return BadRequest(new BaseResponse<object>
            {
                code = HttpStatusCode.BadRequest.GetHashCode().ToString(),
                message =
                    "Token đã hết hạn vui lòng vào mail của bạn để kích hoạt hoặc đăng nhập để nhận lại mail xác nhận mới"
            });
        }

        User? user = _context.Users.FirstOrDefault(u => u.Id == token.UserId);
        if (user == null)
        {
            _logger.LogError($"ACTIVE ACCOUNT TOKEN VALUE: {tokenValue} ERROR >>> USER NOT FOUND");
            return BadRequest(new BaseResponse<object>
            {
                code = HttpStatusCode.BadRequest.GetHashCode().ToString(),
        ...
        {
            code = HttpStatusCode.OK.GetHashCode().ToString(),
    ...
    public IActionResult register([FromBody] RegisterRequest request)
    {
        ...
        if (user != null)
        {
            _logger.LogInformation($"REGISTER WITH USERNAME: {request.Username} >>> FAIL USERNAME EXIST");
            return Ok(new BaseResponse<string>
            {
                code = HttpStatusCode.BadRequest.GetHashCode().ToString(),
        ...
        Role userRole = _context.Roles.FirstOrDefault(role => role.RoleValue.Equals("USER"));
        ...
        User newUser = new User
        {
            ...
            IsLocked = false,
            CreateAt = now,
            RoleValueNavigation = userRole,
            RoleValue = "USER"
        };
        _context.Users.Add(newUser);
        _context.SaveChanges();
        SendActiveAccount(newUser.FullName, newUser.Username, newUser.Id);
        _logger.LogInformation($"REGISTER USERNAME: {request.Username} >>> SUCCESS");
        return Ok(new BaseResponse<string>
        {
            code = HttpStatusCode.OK.GetHashCode().ToString(),
            message = "Tao tai khoan thanh cong vui long vao email de kich hoat tai khoan"
        });
    }

    private void SendActiveAccount(string name, string email, int userId)
    {
        _logger.LogInformation($"START SEND EMAIL: {email}");
        ...
        string mailContent = $"<h1>Chào {name}!</h1>" +
                             $"<p>Chúng tôi ghi nhận bạn đã đăng ký tạo tài khoản Web coffee, để sử dụng trang web vui lòng nhấn vào nút kích hoạt dưới đây</p>" +
                             $"<a href='https://localhost:5000/active-account?tokenValue={activeToken}'>" +
        ...
        _emailService.SendEmailAsync(email, "CHÀO MỪNG BẠN ĐẾN VỚI WEB COFFEE", mailContent);
    }
}